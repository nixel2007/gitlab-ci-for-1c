...
Процедура ВыполнитьОбновлениеБазы()
	...
	//КомандаЗапуска = СтрШаблон(КомандаЗапуска, ОбъединитьПути(ТекущийКаталог(), "src", "cf"), СтрокаИзмененныхФайлов);
	КомандаЗапуска = "/LoadConfigFromFiles ""%1""";
	КомандаЗапуска = СтрШаблон(КомандаЗапуска, ОбъединитьПути(ТекущийКаталог(), "src", "cf"));
